...
! local variables
    ...
    if (pblhfluxmin) then
        write(*,*) 'The SGS_TKE scheme does not support the pblhfluxmin=.true. PBLH diagnostic so far. Aborting.'
        call task_abort()
    end if 
    ...
    DO WHILE (zi(k+1) .LE. 500.)
       qtke  =MAX(tke(i,j,k),0.)   ! maximum QKE
       IF (maxqke < qtke) then
           maxqke = qtke
           ktke = k
       ENDIF
    ...
    DO WHILE (PBLH_TKE .EQ. 0.) 
       ...
       qtke  =MAX(tke(i,j,k),0.)      ! maximum TKE
       qtkem1=MAX(tke(i,j,k-1),0.)
